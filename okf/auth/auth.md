# Auth — Autenticação cliente → Jarvis
...
As keys válidas vêm de `JARVIS_API_KEYS` (lista separada por vírgula). Comparação com `hash_equals` (tempo constante).
...
| [config](../config/config.md) | Lê `JARVIS_API_KEYS` via `Config` |
